...
| **gemba-watchdog** | Bound an agent team's output volume with a deterministic brake a human clears. Use when an agent event chain creates issues, pull requests, or comments faster than a human can read them, when you need a scheduled guardrail that engages an operator latch on a breach, or when you wire the activity watchdog into CI. |
